...
To use MATH::Simplex correctly, the followings are needed, inclusively.
...
The return value of MATH::Simplex is a vector<Type>, 
...
std::vector<D> Simplex(OP f,                                              // target function
    ...
    if (x.size() == 0)  // if no initial simplex is specified
    {
        ...
        std::transform(del.begin(), del.end(), del.begin(), [](D value) { return value / 20; });  //'20' is picked
        ...
        // xcentriod
        std::transform(init.begin(), init.end(), xcentroid_old.begin(), [N](D value) { return value * (N + 1); });
