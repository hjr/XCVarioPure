...
void MpuImu::calculateFactoryReference(vector_f *corr_samp, int nr) {
    ...
    // q_meas rotation from sensor corrected -> sensor
    // reference in sequence with the default mounting position
    // currently measured in the device frame, which is roughly the same as the default glider reference, but without the factory correction
    // sensor_corr * default = default * meas^-1 -> sensor_corr = default * meas^-1 * default^-1 // wanted
    Quaternion q_meas = Quaternion::fromRotationMatrix(X, Y);
    Quaternion q_result = _ref_rot * q_meas.get_conjugate() * _ref_rot.get_conjugate();
    // check q meas
    // vector_f tmp = q_meas.rotate(vector_f(0, 0, 1));
    // ESP_LOGI(FNAME, "Factory reference Z: (%.2f,%.2f,%.2f)", tmp.x, tmp.y, tmp.z);
    // ESP_LOGI(FNAME, "Factory reference quaternion: (%.2f,%.2f,%.2f,%.2f) %f degree", q_meas._w, q_meas._x, q_meas._y, q_meas._z, rad2deg(q_meas.getAngle()));
