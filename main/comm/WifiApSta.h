...
class WifiApSta final : public InterfaceCtrl {
   ...
   public:
    ...
    EventGroupDef_t* getDppEventGroup() const { return _dpp_event_group; }
    void setDppQrCallback(dpp_ota_qr_cb_t cb) { _dpp_qr_callback = cb; }

   private:
    // internal functionality
    enum wifi_mode_t : int8_t { AP = 0, STA = 1, APSTA = 3, STADPP = 4 };
    bool initialize_wifi(wifi_mode_t mode, int maxcon, const char* ssid);
    void client_connect();
    ...
    wifi_mode_t _current_mode = AP;
    // dpp stuff
    EventGroupDef_t* _dpp_event_group = nullptr;
    dpp_ota_qr_cb_t _dpp_qr_callback = nullptr;
};
