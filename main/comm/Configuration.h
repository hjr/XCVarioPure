...
union RoutingTarget {
    ...
    // Convenience ctor's
    constexpr RoutingTarget(DeviceId did, ItfTarget itfp = 0) : raw((did & 0x7f) | (itfp.raw & 0x1ffffff) << 7) {}
    constexpr RoutingTarget(DeviceId did, InterfaceId itf, int port) : raw((did & 0x7f) | ItfTarget(itf, port).raw << 7) {}
    constexpr RoutingTarget(uint32_t r) : raw(r) {}
    // getter
    ...
    constexpr void setItfId(InterfaceId id) {
        intf_port &= ~0x3f;
        intf_port |= (id & 0x3f);
    }  // knowing ItfTarget
    constexpr void setItfPort(int p) {
        intf_port &= ~(0x1fffff << 4);
        intf_port |= ((p & 0x1fffff) << 4);
    }
...
union PackedInt5Array {
    ...
    constexpr int get(int index) const { return (data >> (index * 5)) & 0x1f; }
    constexpr InterfaceId itf(int index) const { return static_cast<InterfaceId>(get(index)); }
    constexpr ProtocolType proto(int index) const { return static_cast<ProtocolType>(get(index)); }
    ...
    // Compare
    constexpr bool operator==(const PackedInt5Array& other) const { return data == other.data; }
};
...
union ProtocolList {
    ...
    // Compare
    constexpr bool operator==(const ProtocolList& other) const { return data == other.data; }
};

//
// Device serialization
//
// the structure belongs through the SetupNG list to a device, the device id is
// contained redundently and does not imply a "never change" (!). It might be
// used for checks. SetupNG variable <--> DeviceNVS DeviceNVS ->
// dev+interface+port and protocols all from ONE data link, potentially
// different ports for NMEA and binary protocols
//              protocol list: 0: the bin proto, 1: the principal nmea 2-5:
//              plugins as far as they are not auto components of the pricipal
//              proto
...
struct DeviceNVS {
    ...
    constexpr DeviceNVS(RoutingTarget t, ProtocolList s, uint16_t bp, uint16_t np) : target(t), setup(s), bin_sp(bp), nmea_sp(np) {
        setup.flags = 1;
    }
    constexpr DeviceNVS(uint32_t t, uint32_t s, uint16_t bp, uint16_t np) : target(t), setup(s), bin_sp(bp), nmea_sp(np) {
        setup.flags = 1;
    }
    ...
    constexpr bool operator==(const DeviceNVS& other) const {
        return target == other.target && setup == other.setup && bin_sp == other.bin_sp && nmea_sp == other.nmea_sp;
    }
};
...
struct DeviceAttributes {
    ...
    union {
        struct {
            ...
            uint8_t aVariant : 1;      // just a variant of the connection details, not a
                                       // new device entry
